    ...
    for _ in range(rounds):
        ...
        if is_python:
            result = subprocess.run(
                [sys.executable, str(executable_or_script), str(n_value)],
                capture_output=True,
                text=True,
                check=False,
            )
            ...
            result = subprocess.run(
                [str(executable_or_script), str(n_value)],
                capture_output=True,
                text=True,
                check=False,
            )
...
def run_performance_tests(generated_files):
    ...
    for algorithm_name, files in generated_files.items():
        ...
        for n in n_values:
            ...
            resultado = extract_result_from_output(
                py_result.stdout if py_result.returncode == 0 else ""
            )

            # 2. Measure C++ Transpiled (with command line argument)
            transpiled_exe = files["executable_transpiled"]
            # TODO(any): trans_result is not used, is it needed?
            tiempo_trans, trans_result = measure_execution_time(
                transpiled_exe, n, is_python=False
            )
            ...
            manual_exe = files["executable_manual"]
            if manual_exe:
                tiempo_manual, _ = measure_execution_time(
                    manual_exe, n, is_python=False
                )

            # Calculate speedup
            speedup_trans = tiempo_py / tiempo_trans if tiempo_trans > 0 else 0
            speedup_manual = (
                tiempo_py / tiempo_manual if tiempo_manual and tiempo_manual > 0 else 0
            )

            # Print result immediately (n by n)
            if tiempo_manual:
                print(
                    f"{n:<6} {resultado:<12} {tiempo_py:<12.3f} {tiempo_trans:<14.3f} {tiempo_manual:<14.3f} {speedup_manual:<10.2f}x"
                )
            else:
                print(
                    f"{n:<6} {resultado:<12} {tiempo_py:<12.3f} {tiempo_trans:<14.3f} {'N/A':<14} {speedup_trans:<10.2f}x"
                )
            ...
            csv_data.append(
                {
                    ...
                    "speedup_manual": (
                        round(speedup_manual, 2) if speedup_manual else None
                    ),
                }
            )
        ...
        try:
            with open(csv_file, "r", encoding="utf-8") as f:
                ...
                if rows:
                    ...
                    speedups_trans = [
                        float(row["speedup_transpiled"])
                        for row in rows
                        if row["speedup_transpiled"]
                    ]
                    speedups_manual = [
                        float(row["speedup_manual"])
                        for row in rows
                        if row["speedup_manual"]
                    ]

                    avg_speedup_trans = (
                        sum(speedups_trans) / len(speedups_trans)
                        if speedups_trans
                        else 0
                    )
                    ...
                    avg_speedup_manual = (
                        sum(speedups_manual) / len(speedups_manual)
                        if speedups_manual
                        else 0
                    )
                    ...
                    print(
                        f"  Transpiled speedup - Avg: {avg_speedup_trans:.2f}x, Max: {max_speedup_trans:.2f}x, Min: {min_speedup_trans:.2f}x"
                    )
                    if speedups_manual:
                        print(
                            f"  Manual speedup     - Avg: {avg_speedup_manual:.2f}x, Max: {max_speedup_manual:.2f}x, Min: {min_speedup_manual:.2f}x"
                        )
                    else:
                        print("  Manual C++: Not available")
        except Exception as e:
