...
from src.benchmarks.file_generator import copy_python_original, copy_cpp_manual, create_transpiled_cpp_file, modify_transpiled_for_args
from src.benchmarks.utilities import generate_n_values_for_algorithm, check_manual_cpp_file_exists


def transpile_file(py_file, output_cpp_file):
    """Transpile a single Python file to C++"""
    cmd = [sys.executable, "-m", "src.tools.transpile_cli", str(py_file), "-o", str(output_cpp_file)]
    result = subprocess.run(cmd, capture_output=True, cwd=PATHS['project_root'])
    return result
...
def compile_cpp_transpiled(cpp_file, executable_path):
    ...
    compile_cmd = [
        "g++", *COMPILE_SETTINGS['cpp_flags'],
        "-I", str(PATHS['project_root'] / COMPILE_SETTINGS['runtime_includes'][0]),
        str(cpp_file),
    ...
    result = subprocess.run(compile_cmd, capture_output=True, cwd=PATHS['project_root'])
    return result
...
def compile_cpp_manual(cpp_file, executable_path):
    ...
    compile_cmd = [
        "g++", *COMPILE_SETTINGS['cpp_flags'],
        str(cpp_file), "-o", str(executable_path)
    ]
    result = subprocess.run(compile_cmd, capture_output=True, cwd=PATHS['project_root'])
    return result
...
def generate_all_transpiled_files(fast_mode=False, max_values=None):
    ...
    for py_file in python_files:
        ...
        print(f"  Test values: {len(n_values)} points ({min(n_values)} to {max(n_values)})")
        ...
        final_transpiled_cpp = create_transpiled_cpp_file(transpiled_cpp, algo_dir, algorithm_name)
        ...
        if compile_result.returncode != 0:
            print(f"  ✗ Transpiled compilation failed: {compile_result.stderr.decode()}")
            continue
        ...
        if manual_exists:
            ...
            if manual_cpp_copy:
                ...
                if manual_result.returncode == 0:
                    ...
                    print(f"  ✗ Manual compilation failed: {manual_result.stderr.decode()}")
                    manual_exe = None
        ...
        print(f"  ✓ Algorithm ready for testing")
