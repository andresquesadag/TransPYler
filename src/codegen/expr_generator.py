...
from src.core import (
    AstNode, LiteralExpr, Identifier,
    UnaryExpr, BinaryExpr, CallExpr, Attribute
)
...
class ExprGenerator:
	...
	def visit(self, node: AstNode) -> str:
		...
		if not m:
			raise NotImplementedError(f"ExprGenerator no soporta nodos de tipo {type(node).__name__}")
		return m(node)
	...
	def visit_LiteralExpr(self, node: LiteralExpr) -> str:
		...
		raise NotImplementedError(f"LiteralExpr con valor de tipo {type(v).__name__} no soportado")	
