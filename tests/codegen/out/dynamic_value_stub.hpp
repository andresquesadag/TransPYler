...
class DynamicValue {
public:
    enum Type { NONE, INT, FLOAT, STRING, BOOL, LIST, DICT };
...
public:
    ...
    std::string toString() const {
        ...
        if (type == LIST) {
            ...
            for (size_t i = 0; i < list_val.size(); i++) {
                out += list_val[i].toString();
                if (i < list_val.size() - 1) out += ", ";
            }
            out += "]";
            return out;
        }
        if (type == DICT) {
            std::string out = "{";
            size_t i = 0;
            for (const auto &p : dict_val) {
                out += p.first + ": " + p.second.toString();
                if (++i < dict_val.size()) out += ", ";
            }
            out += "}";
            return out;
        }
    ...
    bool toBool() const {
        ...
        if (type == DICT) return !dict_val.empty();
        return false;
    }
